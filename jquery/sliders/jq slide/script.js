$(function() {      
  var slider = $('.big-slider'),
    ...
    slideWidth = $('.big-slider-box').outerWidth(),         // Ширина слайдера
    slideCount = $('.big-slider img').length,               // Количество слайдов
    prev = $('.big-slider-box .prev'),                      // Кнопка "назад"
    next = $('.big-slider-box .next'),                      // Кнопка "вперед"
  ...
  $('.big-slider img:last').clone().prependTo('.big-slider');   // Копия последнего слайда помещается в начало.
  $('.big-slider img').eq(1).clone().appendTo('.big-slider');   // Копия первого слайда помещается в конец.  
 // $('.big-slider').css('margin-left', -slideWidth);         // Контейнер .big-slider сдвигается влево на ширину одного слайда.
  ...
  function animate(){
    if (margin==-slideCount*slideWidth-slideWidth){     // Если слайдер дошел до конца
      slider.css({'marginLeft':-slideWidth});           // то блок .big-slider возвращается в начальное положение
      margin=-slideWidth*2;
    }else if(margin==0 && course==-1){                  // Если слайдер находится в начале и нажата кнопка "назад"
      slider.css({'marginLeft':-slideWidth*slideCount});// то блок .big-slider перемещается в конечное положение
      margin=-slideWidth*slideCount+slideWidth;
    }else{                                              // Если условия выше не сработали,
    margin = margin - slideWidth*(course);              // значение margin устанавливается для показа следующего слайда
    }
    slider.animate({'marginLeft':margin},animateTime);  // Блок .big-slider смещается влево на 1 слайд.
  }
